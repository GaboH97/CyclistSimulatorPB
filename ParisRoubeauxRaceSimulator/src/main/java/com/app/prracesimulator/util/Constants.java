/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.app.prracesimulator.util;

/**
 * Clase que contiene las constantes del sistema
 *
 * @author Gabriel Huertas <gabriel970826@gmail.com>
 */
public class Constants {

	public static final String MAX_POW_OUTPUT_FILE_PATH = "./resources/maximal_power_output.csv";
	public static final String PAVE_SECTIONS_DATA_FILE_PATH = "./resources/pave_sections_data_test.csv";
	public static final String CYCLIST_PATH = "src/main/java/img/cyclist.gif";
	public static final String IMG_FINISHER = "src/main/java/img/finish.png";
	public static final String DEQUALIFIED = "src/main/java/img/dequalifiedPro.png";
	public static final String RACING = "src/main/java/img/racing.png";
}
